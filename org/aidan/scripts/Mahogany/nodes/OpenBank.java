...
public class OpenBank extends Node {
    ...
    public boolean activate() {
        ...
        return Banker != null && Inventory.contains(Variables.logs) && Banker.isOnScreen() && !Widgets.get(323, 5).visible();
    }
    ...
    public void execute() {
        ...
        Mahogs.s = "Banking";
        final NPC banker = NPCs.getNearest(BANKER_ID);
        if (banker != null && !Widgets.get(323, 5).visible()) {
            do {
                banker.interact("Send-parcel");
                sleep(2000);
            } while (banker.isOnScreen() && !Widgets.get(323, 5).visible());
        }
    }
}